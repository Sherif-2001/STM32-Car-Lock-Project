...
int main() {
	...
	while (1) {
		...
		current_state2= Gpio_ReadPin(GPIO_A,1);
			  ...
			  case 0:
					GPT_StartTimer(10000);    // start time for anti theft
			...
			if(!locked && closed && GPT_GetElapsedTime()==ambLightDuration)
			{
		        GPIO_WritePinValue(ambient_Led,0);    // Ambient light OFF
		        ambLightDuration=-1;
			}

			if (numberOfBlinks>0 && (GPT_GetElapsedTime()==nextBlinkTime))
			{
				...
				numberOfBlinks--;                     // update number of remaining switches
		     }
